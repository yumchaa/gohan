  ...
  fs.readdirSync(directory).forEach(file => {
    ...
    if ((stat.isFile() && extname === '.vue') || extname === '.we') {
      ...
      if (fileType && extname !== fileType) {
        console.log(
          'Error: This is not a good practice when you use ".we" and ".vue" togither!'
        )
      }
      const name = pathTo.join(dir, pathTo.basename(file, extname))
      if (extname === '.vue') {
        const entryFile = pathTo.join(
          vueWebTemp,
          dir,
          pathTo.basename(file, extname) + '.js'
        )
        fs.outputFileSync(
          pathTo.join(entryFile),
